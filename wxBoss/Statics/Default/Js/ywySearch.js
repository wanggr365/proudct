...
function ywyCustConfirm(_i){
	location.href = "wxBoss/index.php?m=ywy&a=ywyIndex&custCode="+document.getElementById('custCode'+_i).innerHTML;
}

function getYwyBill(){
	location.href = "wxBoss/index.php?m=ywy&a=ywyBill&acctNo="+Request['acctNo'].toString();
}

function getYwyOrding(SUBSCRIBERID){
	location.href = "wxBoss/index.php?m=ywy&a=ywyOrdering&SUBSCRIBERID="+SUBSCRIBERID;
}
...
function goUrl(_i){	
	if($("#custCode").html() != "" ||  _i == 7 ||  _i == 8){
		...
		switch(_i){
			case 1:			
			window.location.href = "http://www.968816.com.cn/wxBoss/index.php?m=ywy&a=ywyAcct&acctNo="+$("#acctNo").html()+"&custCode="+$("#custCode").html();		
			break;
			case 2:			
			window.location.href = "http://www.968816.com.cn/wxBoss/index.php?m=ywy&a=ywyUser&custCode="+$("#custCode").html();	
			break;
			case 3:			
			window.location.href = "http://www.968816.com.cn/wxBoss/index.php?m=ywy&a=ywwOrdering&custCode="+$("#custCode").html();	
			break;
			case 4:			
			window.location.href = "http://www.968816.com.cn/wxBoss/index.php?m=ywy&a=ywyBusiness&custCode="+$("#custCode").html();	
			break;
			case 5:			
			window.location.href = "http://www.968816.com.cn/wxBoss/index.php?m=ywy&a=ywyBill&acctNo="+$("#acctNo").html();
			break;
			case 6:			
			window.location.href = "http://www.968816.com.cn/wxBoss/index.php?m=ywy&a=ywyCharge&custCode="+$("#custCode").html() + "&acctNo="+$("#acctNo").html();	
			break;
			case 7:			
...
function goBack(){
	...
	Request = GetRequest(); 
	window.location.href = "http://www.968816.com.cn/wxBoss/index.php?m=ywy&a=ywyIndex&custCode="+Request['custCode'].toString();*/
	window.history.go(-1);
}
...
function queryCustInfo()
{
	  ...
	  $.ajax({
			type:'post',
			  url: "wxBoss/index.php?m=ywy&a=ywyGetCust",
			  context: document.body,
			  dataType:'json',  
			  ...
			  success: function(json){
					if(json.CODE == "99"){
						...
						location.href = "index.php?m=ywy&a=ywyIndex&custCode=" + json.CUSTOMERCODE;
					}
			  }
		});	
}
...
function unBunding()
{
	  $.confirm('确定解绑吗?解绑后需要重新登记才能使用系统！', function () {
        //$.alert('You clicked OK button!');
		  ...
		  $.ajax({
				type:'post',
				  url: "wxBoss/index.php?m=ywy&a=ywyUnBunding",
				  context: document.body,
				  dataType:'json',  
				  data:"custCode="+BASEtrim(document.getElementById("search").value),
				  success: function(json){
...
function login(){
	...
	$.ajax({
		  type:'post',
		  url: "wxBoss/index.php?m=ywy&a=ywyLoginConfirm",
		  context: document.body,
		  dataType:'json',  
		  ...
		  success: function(json){
				if(json.CODE == "99"){
					...
					$.alert('开始使用系统', '登记成功!', function () {
						location.href = "wxBoss/index.php?m=ywy&a=ywyIndex" ;
					});
				}else if(json.CODE == "2"){
...
function queryByName()
{
	  if(BASEtrim(document.getElementsByName("cust_name")[0].value)=="")
	  {
		  ...
		  $.alert('请输入客户姓名', '错误!');
		   //$("input").focus();
	  	  //document.getElementById('result').innerHTML="请输入客户姓名!";
	  	  return ;
	  }
	  ...
	  $.ajax({
	  	    type:'post',
				  url: "wxBoss/index.php?m=ywy&a=query",
				  context: document.body,
				  dataType:'text',  
