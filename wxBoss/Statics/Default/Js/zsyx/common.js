...
function sendCode(obj){
	...
	if(BASEtrim(cert_no.val()) == "" || BASEtrim(phone.val()) == "" || BASEtrim(password1.val()) == "" || BASEtrim(password2.val()) == "" || org_type.val() == "请选择您的代理类型...") {
		layer.open({
			btn: ['好的'],
		...
		if(result && resultResult && certNoResult){
			doPostBack('index.php?m=agent&a=agentSendSMS',backFunc1,$('#registerForm').serialize());//serialize
		}
	}
}
...
function backFunc1(json){
    ...
    if(json.code == 0){
		...
		setInterval('https://open.weixin.qq.com/connect/oauth2/authorize?appid=wx1ae00a2623048bf1&redirect_uri=http%3a%2f%2fwww.968816.com.cn%2fqzgdwl%2findex.php%3fm%3dlogin%26a%3dindex&response_type=code&scope=snsapi_base&state=agent%26agentRegister#wechat_redirect',3000);
	}else{
		layer.open({
			btn: ['好的'],
			content: '<p style="text-align:center;">' + json.msg +'</p>',
