...
function sendCode(obj){
	...
	if(BASEtrim(name.val()) == "" || BASEtrim(boss_no.val()) == "" || BASEtrim(phone.val()) == "" || BASEtrim(password1.val()) == "" || BASEtrim(password2.val()) == ""  ) {
		layer.open({
			btn: ['好的'],
...
function backFunc1(json){
    ...
    if(json.code == 0){
		...
		setInterval('https://open.weixin.qq.com/connect/oauth2/authorize?appid=wx3a20e613be177269&redirect_uri=http%3a%2f%2fwww.968816.com.cn%2fqzgdwl%2findex.php%3fm%3dlogin%26a%3dindex&response_type=code&scope=snsapi_base&state=agent%26agentLogin#wechat_redirect',3000);
	}else{
		layer.open({
			btn: ['好的'],
			content: '<p style="text-align:center;">' + json.msg +'</p>',
		...
		if(json.code == 1){			
			$("#getVerCode").removeClass("code").addClass('disabled1');
...
function checkbossNo(boss_no){
	var reg = /^[0-9]+$/;
	var result = reg.test(boss_no);
	if(result){
		return true;
	}else{
		layer.open({
			btn: ['好的'],
			content: '<p style="text-align:center;">提示！<br/>工号格式不正确！</p>',
			shadeClose: false
		});
		return false;
	}
